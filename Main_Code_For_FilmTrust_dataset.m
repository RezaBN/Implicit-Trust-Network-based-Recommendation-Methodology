...
%% FilmTrust dataset
  load('FilmTrust.mat');
  load('FilTruTrust.mat');
  RatingData = FilmTrust;
  TrustData = FilTruTrust;
...
%% In this part the constracted trust network is evaluated
   
% Making explicit Trust matrix from dataset

[NumberTrustStatement,~]= size(TrustData);

TrustMatrix=zeros(NumberUser,NumberUser);

for i=1:NumberTrustStatement
    
     UserId1 = TrustData(i,1);  
     UserId2 = TrustData(i,2);    
     TrustStatement = TrustData(i,3);   
     
     TrustMatrix(UserId1,UserId2)=TrustStatement;
       ...
       UN = UN+1;
        for j=1:NumberUser
            ...
            if i~=j
                ...
                if UltiITN(i,j) == 1 && TrustMatrix(i,j) == 1
                
                    CorrectITN = CorrectITN+1; 
                                    
                end

            end
        end
        UsersTrustPrec = [UsersTrustPrec;(CorrectITN/SUTu)];

    end
end
USERC = sum(UsersTrustPrec~=0);
PCTI = ((sum(UsersTrustPrec)) / USERC)*100; 
